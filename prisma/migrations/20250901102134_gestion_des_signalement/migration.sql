...
-- AlterTable
ALTER TABLE `user` MODIFY `status` ENUM('PENDING', 'ACTIVE', 'SUSPENDED', 'BANNED') NOT NULL DEFAULT 'PENDING';

-- AlterTable
ALTER TABLE `userreport` ADD COLUMN `adminNotes` TEXT NULL,
    ADD COLUMN `processedAt` DATETIME(3) NULL,
    ADD COLUMN `processedBy` VARCHAR(36) NULL,
    ADD COLUMN `status` ENUM('PENDING', 'DISMISSED', 'PROCESSED') NOT NULL DEFAULT 'PENDING';
...
-- RenameIndex
ALTER TABLE `userreport` RENAME INDEX `UserReport_reportedUserId_fkey` TO `UserReport_reportedUserId_idx`;

-- RenameIndex
ALTER TABLE `userreport` RENAME INDEX `UserReport_reportingUserId_fkey` TO `UserReport_reportingUserId_idx`;
