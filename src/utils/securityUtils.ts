 ...
 */
export function sanitizeSearchParam(searchParam: unknown): string {
...
export function sanitizeNumericParam(
  ...
  const parsed =
    typeof param === 'string' ? parseInt(param, 10) : Number(param);
 ...
 */
export function sanitizeFloatParam(
  param: unknown,
  defaultValue: number,
  min: number = 0,
  max: number = Number.MAX_SAFE_INTEGER
 ...
 */
export function sanitizeProductName(name: unknown): string {
